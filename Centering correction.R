library(Matrix)
library(lme4)
library(performance) # 用于计算ICC和置信区间
...
# 数据示例（假设数据结构）
data <- data.frame(
  center = rep(1:6, each = 2),  # 6个中心
  treatment = rep(c("mLP", "GnRH-ant"), 6),  # 两组治疗
  responder = c(6, 11, 13, 17, 2, 1, 4, 1, 22, 24, 4, 0),  # 成功例数
  total = c(16, 18, 54, 40, 11, 11, 12, 9, 54, 70, 6, 2)   # 总例数
)

# 拟合GLMM模型（含交互项）
fit_interaction <- glmer(
  cbind(responder, total - responder) ~ treatment * factor(center) + (1 | center),
  family = binomial,
  data = data
)
...
write.csv(b,"C:/Users/CC/Documents/table1.csv")
...
b<-print(a, showAllLevels = TRUE,quote = TRUE, noSpaces = TRUE)
write.csv(b,"C:/Users/CC/Documents/table1-1.csv")
...
p_values <- c(0.283, 0.07, 0.087,0.991)
...
# 示例原始P值
p_values <- c(0.095, 0.273, 0.105, 0.714)
# Benjamini-Hochberg校正
adjusted_p <- p.adjust(p_values, method = "BH")  # 或 method = "fdr"（二者等价）
# 输出结果
data.frame(
...
# 临床妊娠率
p_values <- c(0.105, 0.056, 1, 0.015, 0.949, 0.615, 0.640)
# Benjamini-Hochberg校正
adjusted_p <- p.adjust(p_values, method = "BH")  # 或 method = "fdr"（二者等价）
# 输出结果
data.frame(
